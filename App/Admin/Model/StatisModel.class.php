 ...
 */
  ...
  public function getTotalByMonthByOrg($year){  //默认年份为当前年份
    ...
    "SELECT IFNULL(h_name,'合计') AS h_name,
      ...
      FROM(
          ...
          FROM(
            SELECT h_name,FROM_UNIXTIME(syn_time,'%Y%m')as year,SUM(o.total_money) count
            FROM {$prefix}order o
            JOIN {$prefix}hotel h ON h.id = o.h_id
            WHERE FROM_UNIXTIME(syn_time,'%Y') = $year
  ...
  public function getTotalByMonthByHotel($h_id,$day,$month){  //按照查询条件分别返回数值
    ...
    $condition = "FROM_UNIXTIME(o.syn_time,'%Y%m%d') = $day";
    if($month != "")
    {
      $condition = "FROM_UNIXTIME(o.syn_time,'%Y%m') = $month";
    }
    // dump($condition);die;
    $sql =
   ...
   * 右侧获取分店总计销售详情
   */
  public function getDetailByHotel($h_id,$startTime,$endTime){  
    $prefix = C('DB_PREFIX');
    ...
    $sql =
    "
      SELECT cm.id, cm.menu_name, cm.price price, IFNULL(temp.num,'0') num, IFNULL(price*num,'0') total 
      FROM( 
            SELECT cm.id,cm.menu_name,SUM(num) num
            FROM {$prefix}cook_menu cm
            LEFT JOIN {$prefix}order_sub os ON cm.id = os.menu_id
            LEFT JOIN {$prefix}order o ON o.id = os.oid
            WHERE o.h_id = $h_id and syn_time BETWEEN $startTime AND $endTime
            GROUP BY menu_name
          ) AS temp
  ...
  public function getNumByMonthByOrg($year){  //默认年份为当前年份
    ...
    "SELECT IFNULL(menu_name,'合计') AS menu_name, amount,
      ...
      FROM(
          ...
          FROM(
          SELECT cm.menu_name, FROM_UNIXTIME(syn_time,'%Y%m')as year, SUM(os.num) count, 
              os.price amount
              FROM cloud_order o
...
/**
 ...
 /**
   ...
   */
  public function searchDetailByNumDE($h_id,$startTime,$endTime){  
    $prefix = C('DB_PREFIX');
    $sql =
    "
   ...
   */
  public function searchDetailByNumA($h_id,$startTime,$endTime){  
    $prefix = C('DB_PREFIX');
    $sql =
    "
      SELECT cm.menu_name, cm.price price, IFNULL(temp.num,'0') num, IFNULL(price*num,'0') total 
      ...
      ORDER BY length(num) ASC,num ASC
    ";
    $ret = M("")->query($sql);
    // dump($ret);die();
    return $ret;
   ...
   */
  public function searchDetailByTotalDE($h_id,$startTime,$endTime){  
    $prefix = C('DB_PREFIX');
    $sql =
    "
      SELECT cm.menu_name, cm.price price, IFNULL(temp.num,'0') num, IFNULL(price*num,'0') total 
   ...
   */
  public function searchDetailByTotalA($h_id,$startTime,$endTime){  
    $prefix = C('DB_PREFIX');
    $sql =
    "
      SELECT cm.menu_name, cm.price price, IFNULL(temp.num,'0') num, IFNULL(price*num,'0') total 
...
/**
   * 按查询时间获取销量前五
   */
  public function firstFiveByHotel($h_id,$startTime,$endTime){  
    $prefix = C('DB_PREFIX');
    $sql =
    "
      SELECT cm.menu_name, cm.price price, IFNULL(temp.num,'0') num, IFNULL(price*num,'0') total 
   ...
   * 查询单个菜品某日销量
   */
  public function cookmenuByDay($h_id,$menu_id,$startTime,$endTime){  
    $prefix = C('DB_PREFIX');
    // $sql =
    // "
    //   SELECT cm.id,IFNULL(temp.num,'0') num, IFNULL(price*num,'0') total 
    //   FROM( 
    //         SELECT cm.id,SUM(num) num
    //         FROM {$prefix}cook_menu cm
    //         LEFT JOIN {$prefix}order_sub os ON cm.id = os.menu_id
    //         LEFT JOIN {$prefix}order o ON o.id = os.oid
    //         WHERE o.h_id = $h_id and o.syn_time = $time 
    //       ) AS temp

    //   RIGHT JOIN {$prefix}cook_menu cm ON cm.id = temp.id

    //   WHERE cm.id = $menu_id and cm.org_id = {$_SESSION["org_id"]}
    // ";
    $sql =
    " SELECT cm.id,IFNULL(SUM(num),'0') num, IFNULL(cm.price*num,'0') total 
      FROM {$prefix}cook_menu cm
      LEFT JOIN {$prefix}order_sub os ON cm.id = os.menu_id
      LEFT JOIN {$prefix}order o ON o.id = os.oid
      WHERE o.h_id = $h_id and cm.id = $menu_id and cm.org_id = {$_SESSION["org_id"]} and o.syn_time BETWEEN $startTime and $endTime
    ";
    $ret = M("")->query($sql);
    // dump($ret);die();
    return $ret;
